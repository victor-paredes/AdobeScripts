...
function rebuildTable(){
    ...
    for(var r=startIndex; r<endIndex; r++){
        ...
        for(var f=0; f<featureColumns.length; f++){
            ...
            var featureValue = (csvData[r][featureName] || "").toUpperCase();
            checkbox.value = (featureValue === "X" || featureValue === "1");
...
exportTemplateBtn.onClick = function(){
    ...
    templateFile.writeln("# Required columns: Position, PositionType, Product, SKU, Price");
    ...
    // Export feature colors as metadata
    if(featureColumns.length > 0){
        templateFile.writeln("# Feature Colors (do not delete these lines):");
        for(var i = 0; i < featureColumns.length; i++){
            var colorValue = "";
            if(i < featureColorInputs.length && featureColorInputs[i]){
                colorValue = featureColorInputs[i].text || "#999999";
            } else {
                colorValue = "#999999";
            }
            templateFile.writeln("# " + featureColumns[i] + "=" + colorValue);
        }
        templateFile.writeln("# ");
    } else {
        // Default feature colors for example features
        templateFile.writeln("# Feature Colors (do not delete these lines):");
        templateFile.writeln("# Organic=#F4DDDB");
        templateFile.writeln("# Local=#D3E5F4");
        templateFile.writeln("# Sale=#D7EBE6");
        templateFile.writeln("# ");
    }
    
    // Create header row with current features if any
    var headerRow = "Position,PositionType,Product,SKU,Price,Features";
    if(featureColumns.length > 0){
        for(var i = 0; i < featureColumns.length; i++){
    ...
    if(featureColumns.length > 0){
        // Use actual feature columns - need empty Features column + one value per feature
        var featureCount = featureColumns.length;
        // Row 1: first two features active
        var row1 = "1,Shelf,Example Product 1,SKU001,$3.99,";
        for(var i = 0; i < featureCount; i++){
            row1 += (i < 2) ? ",x" : ",";
        }
        templateFile.writeln(row1);
        
        // Row 2: first and last feature active
        var row2 = "1,Shelf,Example Product 2,SKU002,$4.49,";
        for(var i = 0; i < featureCount; i++){
            row2 += (i === 0 || i === featureCount - 1) ? ",x" : ",";
        }
        templateFile.writeln(row2);
        
        // Row 3: only last feature active
        var row3 = "2,Peg,Example Product 3,SKU003,$2.99,";
        for(var i = 0; i < featureCount; i++){
            row3 += (i === featureCount - 1) ? ",x" : ",";
        }
        templateFile.writeln(row3);
        
        // Row 4: only second feature active
        var row4 = "2,Peg,Example Product 4,SKU004,$3.49,";
        for(var i = 0; i < featureCount; i++){
            row4 += (i === 1) ? ",x" : ",";
        }
        templateFile.writeln(row4);
    } else {
        // Default example with 3 features (Organic, Local, Sale)
        templateFile.writeln("1,Shelf,Milk 2%,SKU001,$4.99,,x,x,");
        templateFile.writeln("1,Shelf,Almond Milk,SKU002,$5.49,,x,,x");
        templateFile.writeln("2,Peg,Cereal Box,SKU003,$3.99,,,,x");
        templateFile.writeln("2,Peg,Granola Bar,SKU004,$4.49,,,x,");
    }
